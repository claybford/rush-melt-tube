...
async function getVideoTranscript() {
  ...
  let selectedTrack = captionTracks.find((track) => track.languageCode === "en" && !track.kind);
  if (!selectedTrack) {
    selectedTrack = captionTracks.find((track) => track.languageCode === "en" && track.kind === "asr");
  }
  if (!selectedTrack) {
    throw new Error("No suitable caption track found (manual or auto-generated).");
  }
...
async function sendToOpenAI(text) {
    try {
      ...
      const chunkTasks = chunks.map((chunk, index) => {
        ...
        return async () => {
          try {
            ...
            if (!response.ok) {
              throw new Error(`API error for chunk ${chunkNumber}: ${response.status} - ${await response.text()}`);
            }
      ...
      const summaryResults = await runWithConcurrencyLimit(chunkTasks, concurrencyLimit);
      ...
      const combinedSummary = await combineParallelSummaries(orderedSummaries, settings);
...
function chunkText(text, maxTokens) {
  ...
  async function createPopup(message) {
    ...
    try {
      ...
      const halfViewport = Math.max(300, Math.min(800, Math.floor(window.innerWidth / 2)));
      ...
      function updatePopupWidth() {
        const newHalfViewport = Math.max(300, Math.min(800, Math.floor(window.innerWidth / 2)));
  ...
  function parseMarkdown(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (headerMatch) {
            ...
            const content = processInlineFormatting(escapeHtml(headerMatch[2].trim()));
